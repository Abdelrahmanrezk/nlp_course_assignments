...
class FeatureFactory:
    ...
    def wordCase(self, word):
        if word.isupper():
            return "UPPERCase"
        elif word.islower():
            return "lowercase"
        elif re.match('[A-Z][a-z]+', word): #check captilze wors
            return "CapitalizeCase"
        elif re.match('[a-z]+([A-Z][a-z]+)+', word): #check camel 
            return "CamelCase"
        elif re.match('^[A-Za-z]+-[A-Za-z]+$', word):
            return "dashCase"
        else: return "OtherCase"
    def hasDigit(self, word):
        for c in word:
            if c.isdigit():
                return "True"
        else: return "False"
	...
	""" TODO: Add your features here """
        features.append("hasDigit="+self.hasDigit(currentWord))
        features.append("len=" + str(len(currentWord)))
        features.append("wordPrefix=" + self.wordPrefix(currentWord))
        features.append("wordSuffix=" + self.wordSuffix(currentWord))

        wp = self.wordCase(currentWord)
        if wp == 'CapitalizeCase' or wp == 'dashCase':
