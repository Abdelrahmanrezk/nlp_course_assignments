...
class IRSystem:
    ...
    def compute_tfidf(self):
        ...
        self.doc_tfidf = defaultdict(float)
        # for doc in self.docs:
        len_documents= len(self.docs)
        for word in self.vocab:
            word_idf = math.log10(float(len_documents)/len(self.get_posting(word)))
            for d in range(len_documents):
                try:
                    self.tfidf[d][word] = (1.0+math.log10(self.tf[d][word])) * word_idf
                except ValueError:
    ...
    def index(self):
        ...
        for indx, doc in enumerate(self.docs):
            for word in doc:
                inv_index[word].add(indx)  # for each word add document indx has
                self.tf[indx][word] +=1 # for each word count its occurance in the indx document
    ...
    def get_posting(self, word):
        ...
        # print(word, "ddddddd")
        posting = list(self.inv_index[word])
        # print(posting, "========")
    ...
    def boolean_retrieve(self, query):
        ...
        #         if query[indx] in 
        #     for indx, key in enumerate(self.inv_index)
        #         if self.inv_index[key]
        #         docs.append(d)
        # for key in self.inv_index:
        #     for x in self.inv_index[key]:
        #         print(x)

        # sets = set()
        previous_intrsect = set(self.get_posting(query[0]))
        for indx in range(1,len(query)):
            previous_intrsect = set(self.get_posting(query[indx])).intersection(previous_intrsect)
            # print(self.get_posting(query[indx]))
            # if q in self.inv_index.keys():
                # lis = list(self.inv_index[q])
                # for i in lis:
                    # sets.add(i)
                    # docs.append(i)
                # print(type(lis))
                # docs.append(lis)
                # print(lis)
                # docs.append(list(self.inv_index[q]))
                # print("fff")
                # docs.append(self.inv_index[q])
        #     docs.append(self.inv_index[q])
        #     print(self.inv_index[q])
        #     if self.inv_index[q]:
        #         for indx, key in enumerate(self.inv_index[q]):
        #             docs.append(self.inv_index[q][indx])

            # print(q)
        # docs = list(sets)
        # print(docs)
        # ------------------------------------------------------------------
        # print(previous_intrsect)
        return sorted(list(previous_intrsect))   # sorted doesn't actually matter
